#!/usr/bin/env python3
"""
PROPERLY Fixed Trained Agent Recorder
Uses EXACT same environment setup as training
"""
import os
...
def main():
    ...
    output_path = '/data/workspace/trained_agent_proper.mp4'
    ...
    # CRITICAL: Same as train_sb3.py line 145-146
    ...
    print("✅ Environment wrapped with VecTransposeImage")
    
    print("\nResetting environment...")
    obs = vec_env.reset()
    print(f"Observation shape: {obs.shape} (should be (1, 1, 72, 80) for CHW)")
    ...
    print(f"\nRecording 1000 frames to {output_path}...")
    actions_taken = []
    
    for i in range(1000):
        # Model expects batched, CHW format
        action, _ = model.predict(obs, deterministic=False)
        action_idx = int(action[0])
        actions_taken.append(action_idx)
        
        # Get frame for video (from unwrapped env)
        frame = env.get_frame_rgb()
    ...
    # Check if it's actually learning (should have variety)
    unique_actions = len(set(actions_taken))
    if unique_actions < 3:
        print("\n⚠️  WARNING: Model is mostly using same action - may not be working correctly")
    else:
        print(f"\n✅ Model using {unique_actions}/9 different actions - looks good!")
